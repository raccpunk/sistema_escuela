...
class KardexController extends Controller
{
    ...
    public function KardexPrimeroDocWord($alumno, $grado, $grupo, $ciclo_escolar)
    {
        ...
        foreach ($mapaDelGradoYCiclo as $mapa) {
            array_push($materias, $mapa->asignatura);
        }
        $contarmat=count($materias);
        ...
        if(!is_null($promediogeneral)){
            $table->addCell(500)->addText($promediogeneral/$contarmat);
        }else{
            $table->addCell(500)->addText("");
...
//    }
    ...
    public function GetAlumnosGrupo($grado, $grupo,$ciclo_escolar)
    {
        $phpWord = new \PhpOffice\PhpWord\PhpWord();
        // Creating the new document...
        $section = $phpWord->addSection(array('orientation' => ''));
        ...
        return response()->download('Kardex.docx')->deleteFileAfterSend(true);
    }
}
